...
class Category(models.Model):
    '''投稿する写真のカテゴリを管理するモデル
    '''
    # カテゴリ名のフィールド カテゴリの列を作る idという列が自動的に出る
...
class PhotoPost(models.Model):
    ...
    # 1対多の関係で結び付ける
    # CustomUserが親でPhotoPostが子の関係となる　Foreigkeyは外部キー　userという列は外部キー
    user = models.ForeignKey(
        CustomUser,#主キーがとなるものの票をここに書く。今回はCustomUser
        # フィールドのタイトル
        verbose_name='ユーザー',
        # ユーザーを削除する場合はそのユーザーの投稿データもすべて削除する
        on_delete=models.CASCADE#外部キーの列にこの文字を必ず入れる。
        )
    # Categoryモデル(のtitle)とPhotoPostモデルを
    # 1対多の関係で結び付ける
    # Categoryが親でPhotoPostが子の関係となる
    category = models.ForeignKey(#外部キー
        Category,
        # フィールドのタイトル
        verbose_name='カテゴリ',
        # カテゴリに関連付けられた投稿データが存在する場合は
        # そのカテゴリを削除できないようにする
        on_delete=models.PROTECT#消そうとしても整合性をとるため消せないように
        )
    # タイトル用のフィールド
    title = models.CharField(
    ...
    # イメージのフィールド1
    #Imageフィールドを使う場合は、アップロードの場所を指定
    image1 = models.ImageField(
        verbose_name='イメージ1',# フィールドのタイトル
        upload_to = 'photos'   # MEDIA_ROOT以下のphotosにファイルを保存 ※大事 
        )
    # イメージのフィールド2
