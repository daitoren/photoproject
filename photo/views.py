...
class IndexView(ListView):
    ...
    # 投稿日時の降順で並べ替える
    queryset = PhotoPost.objects.order_by('-posted_at')#必要
    # 1ページに表示するレコードの件数
    paginate_by = 9

# デコレーターにより、CreatePhotoViewへのアクセスはログインユーザーに限定される
# ログイン状態でなければsettings.pyのLOGIN_URLにリダイレクトされる
@method_decorator(login_required, name='dispatch')#セキュリティ的に必須
...
class MypageView(ListView):
    ...
    def get_queryset(self):
        ...
        #filter(userフィールドを=userオブジェクト)で絞り込む
...
class PhotoDeleteView(DeleteView):
    ...
    Attributes:
        model: モデル
        template_name: レンダリングするテンプレート
        peginate_by: 1ページに表示するレコードの件数
        succcess_url: 削除完了後のリダイレクト先のURL
    '''
    #操作の対象はPhotoPostモデル
    model = PhotoPost
    #photo_deldete.htmlをレンダリングする
